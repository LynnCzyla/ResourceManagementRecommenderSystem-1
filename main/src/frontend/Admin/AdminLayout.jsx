...
export default function AdminLayout({ user, onLogout, isDark, toggleTheme }) {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'user-roles':
        return <UserManagementTab activeSubTab="roles" />;
          ...
          <div>
            ...
            {userMgmtOpen && (
              <div style={sidebarCollapsed ? styles.collapsedSubmenu : styles.submenu}>
                ...
                <div 
                  onClick={() => handleNavClick('user-roles')} 
                  style={{
                    ...styles.submenuItem,
                    color: activeTab === 'user-roles' ? 'var(--color-primary)' : 'var(--color-text-secondary)',
                    fontWeight: activeTab === 'user-roles' ? '700' : '400'
                  }}
                  className="hover-submenu-item"
                >
                  {!sidebarCollapsed ? '• User Roles' : 'Roles'}
                </div>
              </div>
            )}
          </div>
